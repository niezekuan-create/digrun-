...
// ── 列表 ──────────────────────────────────────────────────
export const getMockActivitiesList = (
	_clubId?: string,
	...
	const filtered =
		status === "ended"
			...
			: status === "upcoming"
				? ALL_MOCK_ACTIVITIES.filter(
						(a) => a.start === undefined || a.start >= n,
					)
				: ALL_MOCK_ACTIVITIES;
...
export const getMockActivityDetail = (
	id: string,
): { data: MockActivityDetail } | null => {
