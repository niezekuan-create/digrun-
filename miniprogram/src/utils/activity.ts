...
export const MINI_ACTIVITY_DISPLAY_STATUS_LABELS: Record<
	MiniActivityDisplayStatus,
	string
> = {
...
const pickAvatarUrls = (value: any): string[] => {
	const candidates = [value?.joinAvatars];
...
export const getActivityJoinCount = (value: any): number | undefined => {
	return toNumber(value?.joinCount);
};
...
export const normalizeActivityDetail = <T extends Record<string, any>>(
	value: any,
): T | null => {
...
export const dedupeActivities = <T extends { id?: string | number }>(
	list: T[],
) => {
...
export const getActivityListButtonState = (
	activity: ActivitySummary,
): ActivityListButtonState => {
	const rawStatus =
		typeof activity.displayStatus === "string"
			? activity.displayStatus.trim().toLowerCase()
			: "";
	const status = rawStatus as MiniActivityDisplayStatus;
	const hasMappedStatus =
		rawStatus &&
		Object.prototype.hasOwnProperty.call(
			MINI_ACTIVITY_DISPLAY_STATUS_LABELS,
			status,
		);
	const canClick = hasMappedStatus
		? status === "register_open" || status === "waitlist"
		: !!activity.appliable;
...
export const getActivityCtaState = (activity: {
	...
	if (
		...
		if (status === "checkin_pending")
			return {
				text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.checkin_pending,
				enabled: true,
				action: "checkin",
			};
		if (status === "checked_in")
			return {
				text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.checked_in,
				enabled: false,
				action: "none",
			};
		if (status === "register_open") {
			if (isSignedUp)
				return {
					text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.registered,
					enabled: false,
					action: "none",
				};
			return {
				text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.register_open,
				enabled: true,
				action: "signup",
			};
		}
		if (status === "waitlist") {
			...
			return {
				text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.waitlist,
				enabled: true,
				action: "signup",
			};
		}
		if (status === "in_progress") {
			...
			return {
				text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS.in_progress,
				enabled: false,
				action: "none",
			};
		}
		return {
			text: MINI_ACTIVITY_DISPLAY_STATUS_LABELS[status],
			enabled: false,
			action: "none",
		};
	}
	...
	if (
		typeof activity.btnText === "string" ||
		typeof activity.btnStatus === "boolean"
	) {
		const enabled = !!activity.btnStatus;
		const text = String(
			activity.btnText || activity.applyText || (enabled ? "报名" : "已截止"),
		);
	...
	if (end !== undefined && now > end)
		return { text: "已结束", enabled: false, action: "none" };
	...
	if (applyStart !== undefined && now < applyStart)
		return { text: "未开始", enabled: false, action: "none" };

	const inApplyWindow =
		applyStart !== undefined && applyEnd !== undefined
			? now >= applyStart && now <= applyEnd
			: undefined;

	if (inApplyWindow !== false) {
		if (isSignedUp) return { text: "已报名", enabled: false, action: "none" };
		if (activity.appliable)
			return { text: "报名", enabled: true, action: "signup" };
		return {
			text: activity.applyText || "已截止",
			enabled: false,
			action: "none",
		};
	}
