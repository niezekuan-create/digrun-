...
interface EventDetail {
	id: number | string;
	title: string;
	form_config?: { fields: FieldConfig[] };
}
...
export default function RegisterPage() {
	...
	useLoad(async () => {
		...
		try {
			const data = await request<EventDetail>({
				url: `/activities/detail/${eventId}`,
				auth: false,
			});
			...
			setEvent({
				...
				form_config: detail.form_config,
			});
		} catch (e) {
			...
			<ScrollView scrollY className="reg-scroll">
				<View className="reg-body">
					...
					<View className="submit-section">
						<View
							className={`reg-submit-btn${submitting ? " loading" : ""}`}
							onClick={handleSubmit}
						>
							<Text className="reg-submit-text">
