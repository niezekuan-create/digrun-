...
export default function LeaderboardPage() {
	...
	const loadLeaderboard = async () => {
		try {
			const data = await request<LeaderEntry[]>({
				url: "/leaderboard",
				auth: false,
			});
			setEntries(data);
				...
				<>
					{myRank && (
						<View className="my-rank-bar">
							...
							<Text className="my-rank-count">
								{myRank.checkin_count} 次签到
							</Text>
						</View>
					)}
					...
					<ScrollView scrollY className="leader-list">
						{entries.map((entry) => {
							...
							return (
								<View
									key={entry.userId}
									className={`leader-row ${isMe ? "is-me" : ""}`}
								>
									<View
										className={`rank-badge ${isTop3 ? `rank-${entry.rank}` : ""}`}
									>
