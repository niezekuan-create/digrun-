...
const normalizeMyActivity = (value: any): MyActivity => ({
	...
	displayStatus: cleanText(
		value?.displayStatus ?? value?.display_status,
	).toLowerCase(),
});

const normalizeLeaderboardRows = (
	list: LeaderboardApiItem[],
	startRank: number,
): ClubLeaderboardRow[] =>
	list.map((item, index) => ({
...
const getActivityLocationText = (activity: MyActivity) =>
	cleanText(activity.address) ||
	cleanText(activity.city) ||
	cleanText(activity.province);

const getActivityDateText = (activity: MyActivity) =>
	cleanText(activity.timeStr) || cleanText(activity.subtitle);
...
const canCancelSignup = (activity: MyActivity) => {
	...
	return PRE_START_CANCELABLE_STATUSES.has(
		cleanText(activity.displayStatus).toLowerCase(),
	);
};
...
export default function MyPage() {
	...
	const loadAll = async () => {
		...
		try {
			const res = await request<ApiRes<MiniUserProfile>>({
				url: `/api/mini/user/info?clubId=${clubId}`,
			});
	...
	const handleCancelSignup = async (activity: MyActivity) => {
		...
		try {
			...
			if (res?.err)
				throw new Error(res?.msg || res?.message || "cancel_signup_failed");
	...
	const handleScanVerify = async () => {
		...
		setScanLoading(true);
		try {
			const scanRes = await new Promise<Taro.scanCode.SuccessCallbackResult>(
				(resolve, reject) => {
					Taro.scanCode({
						onlyFromCamera: true,
						scanType: ["qrCode"],
			...
			try {
				...
				if (res?.err)
					throw new Error(res?.msg || res?.message || "verify_failed");
				Taro.showToast({
					title: res?.msg || res?.message || "核销成功",
					icon: "success",
				});
			} catch (e: any) {
	...
	const displayName =
		(user as any)?.nickname || (user as any)?.username || username || "跑者";
		...
		<View className="my-page">
			...
			<View className="profile-header">
				...
				<View className="header-actions">
					...
					{showAdmin && (
						<View
							className={`scan-btn${scanLoading ? " scan-btn-loading" : ""}`}
							onClick={handleScanVerify}
						>
							<Text className="scan-text">
								{scanLoading ? "扫码中..." : "扫码核销"}
							</Text>
						</View>
					)}
					{hasToken ? (
				...
				<View className="regs-content">
					{actLoading && myActivities.length === 0 ? (
						...
						<View className="empty">
							...
							<View
								className="go-events-btn"
								onClick={() => Taro.redirectTo({ url: "/pages/events/index" })}
							>
								...
								return (
									<View key={activity.id} className="reg-card">
										<View
											...
											onClick={() => goEventDetail(activity.id)}
										>
											...
											<View className="reg-event-info">
												<Text className="reg-event-title">
													{activity.name || "活动"}
												</Text>
											...
											<View className={`reg-status ${statusInfo.cls}`}>
												<Text className="reg-status-text">
													{statusInfo.label}
												</Text>
											</View>
										</View>
							...
							{orders.map((order) => (
								<View key={order.id} className="order-card">
									<View className="order-img-wrap">
										{order.product?.image ? (
											<Image
												src={normalizeImageUrl(order.product.image)}
												className="order-img"
												mode="aspectFill"
												lazyLoad
											/>
									...
									<View className="order-info">
										...
										{order.size && (
											<Text className="order-meta">尺码：{order.size}</Text>
										)}
										<Text className="order-meta">
											{order.delivery_type === "shipping" ? "邮寄" : "活动自提"}
										</Text>
								...
								return (
									...
									>
										...
										<View className="lb-avatar-wrap">
											{entry.avatar ? (
												<Image
													src={normalizeImageUrl(entry.avatar)}
													className="lb-avatar"
													mode="aspectFill"
													lazyLoad
												/>
												...
												<View className="lb-avatar-placeholder">
													<Text className="lb-avatar-text">
														{entry.username?.[0] || "?"}
													</Text>
												</View>
											)}
										</View>
										<Text className="lb-name">
											{isMe ? `${entry.username} (我)` : entry.username}
										</Text>
