					...
					<View className="balance-card">
						...
						<View className="balance-row">
							...
							<Text className="total-num">
								{points?.points_total ?? 0} 积分
							</Text>
						</View>
						<View className="balance-actions">
							<View
								className="action-btn"
								onClick={() => Taro.navigateTo({ url: "/pages/points/mall" })}
							>
								<Text className="action-btn-text">去商城兑换 →</Text>
							</View>
							<View
								className="action-btn secondary"
								onClick={() => Taro.navigateTo({ url: "/pages/points/orders" })}
							>
								<Text className="action-btn-text secondary-text">
									兑换记录 →
								</Text>
							</View>
						</View>
						...
						{transactions.length === 0 ? (
							...
							transactions.map((txn) => (
								<View key={txn.id} className="txn-item">
									...
									<Text
										className={`txn-change ${txn.points_change > 0 ? "positive" : "negative"}`}
									>
										{txn.points_change > 0 ? "+" : ""}
										{txn.points_change}
									</Text>
