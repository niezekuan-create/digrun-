...
import { request, CLUB_ID_CONFIG, normalizeImageUrl } from '../../utils/request'
import {
...
export default function EventsPage() {
  ...
  const fetchActivities = async (statuses: string[]) => {
    const groups = await Promise.all(
      statuses.map(async (status) => {
        const res = await request<ActivityListResponse>({ url: buildActivitiesUrl(clubId, status) })
  ...
  const loadTab = async (t: TabKey) => {
    ...
    try {
      ...
      const data = rawList.map(normalizeActivitySummary).filter((it) => !!it.id)
      setActivities(data)
    } catch {
        ...
        <View className='event-info'>
          ...
          <View className='info-meta-row'>
            ...
            <View className='meta-item flex-2'>
              ...
              <Text className='meta-value' numberOfLines={1}>{locationLabel}</Text>
              {!!activity.address && (
